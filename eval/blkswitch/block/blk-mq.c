...
retry:
	...
	if (blk_switch_request(data->bio, data)) {
		...
		if (blk_switch_debug && blk_switch_reset_metrics &&
		   blk_switch_stats_print[task_cpu(current)]) {
			printk("(pid %d cpu %2d) reqstr stats: gen %8d str %8d prc %8d",
				current->pid, task_cpu(current),
				blk_switch_stats_gen[task_cpu(current)],
				blk_switch_stats_str[task_cpu(current)],
				blk_switch_stats_prc[task_cpu(current)]);

			blk_switch_stats_gen[task_cpu(current)] = 0;
			blk_switch_stats_str[task_cpu(current)] = 0;
			blk_switch_stats_prc[task_cpu(current)] = 0;
			blk_switch_stats_print[task_cpu(current)] = 0;
		}
	}
	...
	if (blk_switch_on >= 2 && blk_switch_request(data->bio, data)) {
		...
		if (blk_switch_last_appstr == 0)
			...
			if (blk_switch_debug == 3)
				printk("(pid %d cpu %d) -(%d)- %s [0](%lu %lu) [4](%lu %lu) [8](%lu %lu) [12](%lu %lu) [16](%lu %lu) [20](%lu %lu)",
					current->pid, task_cpu(current), blk_switch_appstr_app,
		...
		if (target_cpu >= 0) {
			...
			if (!blk_switch_is_thru_request(data->bio) &&
				READ_ONCE(data->hctx->tags->active_queues) <= 1)
				blk_switch_L_metric[cur_cpu] = 0;
		...
		if (task_cpu(current) != cur_cpu) {
			data->ctx = per_cpu_ptr(q->queue_ctx, task_cpu(current));
			data->hctx = blk_mq_map_queue(q, data->cmd_flags, data->ctx);
			app_steered = true;
		}
	}
