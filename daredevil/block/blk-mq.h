...
#ifdef CONFIG_DAREDEVIL_IO_STACK
	struct blk_blex_rw_stats __percpu *rw_stats;
	/** @blex_hctxs: The default HCTX of this SQ for each priority. */
	struct blk_mq_hw_ctx *blex_hctxs[BLK_BLEX_PRIO_LEVEL];
#endif
} ____cacheline_aligned_in_smp;
...
#ifdef CONFIG_DAREDEVIL_IO_STACK
	/* We add the bio field to the alloc_data struct 
	 * because in blk-blex, determining HQ needs bio info.*/
	struct bio *bio;
#endif
