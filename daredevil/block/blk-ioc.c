...
#ifdef CONFIG_DAREDEVIL_IO_STACK
	struct blk_blex_task_ioc *task_ioc;
...
#ifdef CONFIG_DAREDEVIL_IO_STACK
	/* Disallocate the blex_iocs when exiting. */
	xa_for_each(&ioc->blk_blex_iocs, q_id, task_ioc)
		blk_blex_free_task_ioc(task_ioc);
#endif
}
...
#ifdef CONFIG_DAREDEVIL_IO_STACK
	old_ioprio = task->io_context->ioprio;
#endif
...
#ifdef CONFIG_DAREDEVIL_IO_STACK
	/* Notify the underlying of the potential changes in ioprio. */
	if (!new_alloc && old_ioprio != ioprio){
		struct blk_blex_task_ioc *task_ioc;
		unsigned long q_id = 0;

		xa_for_each(&task->io_context->blk_blex_iocs, q_id, task_ioc){
			WRITE_ONCE(task_ioc->need_update, true);
		}
	}
#endif
	return 0;
